...
suppressWarnings(source("C:/Users/Stefania/Documents/BDA/GroupProject--IMDB/AnalyticsLibraries/library.R"))
# Package options
...
numeric_projectdata = cbind(as.numeric(ProjectData[,"movie_ID"]),hastopactor1,hastopactor2,hastopactor3,hastopdirector,net_USD,as.numeric(ProjectData[,"color"]),as.numeric(ProjectData[,"gross_USD"]),as.numeric(ProjectData[,"cast_total_facebook_likes"]),as.numeric(ProjectData[,"facenumber_in_poster"]),as.numeric(ProjectData[,"title_year"]),as.numeric(ProjectData[,"actor_1_facebook_likes"]),as.numeric(ProjectData[,"actor_2_facebook_likes"]),as.numeric(ProjectData[,"actor_3_facebook_likes"]),as.numeric(ProjectData[,"movie_facebook_likes."]),as.numeric(ProjectData[,"num_voted_users"]),as.numeric(ProjectData[,"duration"]),as.numeric(ProjectData[,"budget_USD"]),as.numeric(ProjectData[,"profitable"]),as.numeric(ProjectData[,"director_facebook_likes"]),as.numeric(ProjectData[,"num_user_for_reviews"]))
...
<hr>\clearpage
#Part 1: making the data numeric
```{r}
datafile_topnames = "Dataset/top20actors&directors.csv"
Topnames <- read.csv(datafile_topnames)
'Topnames <- data.matrix(Topnames)
...
# Please use numbers, not column names, e.g. c(1:5, 7, 8) uses columns 1,2,3,4,5,7,8
segmentation_attributes_used = c(28,19,14,10,3) #c(10,19,5,12,3) 
